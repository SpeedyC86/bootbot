...
class BootBot extends EventEmitter {
  ...
  constructor(options) {
    ...
    if (!options || (options && (
      ...
      !options.appID || !options.appInboxID || !options.appHandoverNotifyUserId))) {
      throw new Error('You need to specify an accessToken, verifyToken, appSecret, appID, appInboxID, appPrimaryUserId, appHandoverNotifyUserId');
    }
    this.accessToken = options.accessToken;
    this.verifyToken = options.verifyToken;
    this.appSecret = options.appSecret;
    this.appID = options.appID;
    this.appInboxID = options.appInboxID;
    this.appHandoverNotifyUserId = options.appHandoverNotifyUserId;
    ...
    this._hearMap = [];
    this._conversations = [];
    this._conversations_owners = {};
    this._conversations_pass_tread_ignore = {};
  }
  ...
  sendRequest(body, endpoint, method) {
    ...
    method = method || 'POST';
    //console.log(`https://graph.facebook.com/${this.graphApiVersion}/me/${endpoint}?access_token=${this.accessToken}`);
   ...
   * @param {String} message to be passed to the secondary account.
   */
  async passThreadToInbox(recipientId, message) {
    const recipient = this._createRecipient(recipientId);

    const reqBody = {
      recipient,
      "target_app_id": this.appInboxID,
      "metadata": message || 'The bot requested your intervention'
    };

    await this.sendRequest(reqBody, 'pass_thread_control');
    this.say(this.appHandoverNotifyUserId, message || 'The bot requested your intervention');
   ...
   * @param {String} message to be passed to the secondary account.
   */
  takeThreadControl(recipientId) {
    const recipient = this._createRecipient(recipientId);

    const reqBody = {
      recipient,
      "metadata": 'The bot restarted and is taking back control'
    };

    return this.sendRequest(reqBody, 'take_thread_control');
  }
  /**
   * @param {String} recipientId The recipient id.
   * @param {String} message to be passed to the secondary account.
   */
  async getThreadOwner(recipientId) {
    if(!this._conversations_owners.hasOwnProperty(recipientId)){
      try{
        console.log(`Thread owner not found, retake ownership`);
        await this.takeThreadControl(recipientId);
        this._conversations_pass_tread_ignore[recipientId] = new Date()
        this._conversations_owners[recipientId] = this.appID;
      } catch(e) {
        console.log(`Error processing message: ${e}`)
        return this.appInboxID;
      }
    }
    return '' + this._conversations_owners[recipientId];
  }
  ...
  say(recipientId, message, options) {
    this.emit("onPreSay", recipientId, message);
  ...
  handleFacebookData(data) {
    ...
    data.entry.forEach((entry) => {
      ...
      entry.messaging.forEach(async (event) => {
        ...
        if (event.message || event.read) {
          const recipient = event.sender.id;
          if(await this.getThreadOwner(recipient) === this.appInboxID){
            console.log("The bot is not the current thread owner, don't process messages");
            return;
          }
        }
        ...
        if (event.optin) {
          ...
          event.message.text = ASCIIFolder.fold(event.message.text)
          ...
          this._handleEvent('pass_thread_control', event);
          // { recipient: { id: '419890095212360' },
          //   timestamp: 1544189133701,
          //   sender: { id: '2325048027523771' },
          //   pass_thread_control:
          //   { new_owner_app_id: 1169338003219368,
          //   metadata: 'Pass thread control from Page Inbox' } }
          const recipient = event.sender.id;
          let ignorePassThreadControl = true;
          if((new Date() - this._conversations_pass_tread_ignore[recipient])>5000){
            ignorePassThreadControl = false;
            this._conversations_owners[recipient] = '' + event.pass_thread_control.new_owner_app_id;
          }
          console.log(`Pass thread control: ${JSON.stringify(event)}\r\n${ignorePassThreadControl}\r\n${await this.getThreadOwner(recipient)}`)
        }else if(event.request_thread_control){
          // { recipient: { id: '419890095212360' },
          //   timestamp: 1544189181673,
          //   sender: { id: '2325048027523771' },
          //   request_thread_control: { requested_owner_app_id: 263902037430900, metadata: null } }
          const recipient = event.sender.id;
          this._conversations_owners[recipient] = '' + event.request_thread_control.requested_owner_app_id;
          this._conversations_pass_tread_ignore[recipient] = new Date()
          console.log(`Request thread control: ${JSON.stringify(event)}\r\n${await this.getThreadOwner(recipient)}`)
        }else if(event.take_thread_control){
          // {"recipient":{"id":"1797497293839760"},"timestamp":1544869860601,
          // "sender":{"id":"2511804495503125"},
          // "take_thread_control":{"previous_owner_app_id":1075467075995159,"metadata":"The bot restarted and is taking back control"}}

          const recipient = event.sender.id;
          this._conversations_owners[recipient] = '' + event.take_thread_control.previous_owner_app_id;
          console.log(`Request thread control: ${JSON.stringify(event)}\r\n${await this.getThreadOwner(recipient)}`)
        } else {
