...
class BootBot extends EventEmitter {
  ...
  constructor(options) {
    ...
    if (!options || (options && (
      ...
      !options.appID || !options.appInboxID || !options.appSecondaryUserId))) {
      throw new Error('You need to specify an accessToken, verifyToken, appSecret, appID, appInboxID, appSecondaryUserId');
    }
    ...
    this.appSecondaryUserId = options.appSecondaryUserId;
  ...
  async passThreadToInbox(recipientId, message) {
    ...
    this.say(this.appSecondaryUserId, message || 'The bot requested your intervention');
